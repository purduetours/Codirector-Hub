...
export async function api(action, payload = {}) {
  ...
  // "Try again" is safe advice for a read and dangerous for a write. Apps Script
  // routinely runs the request and then loses the reply on its redirect hop, so a
  // failed write has very possibly landed — and a rollover run twice moves every
  // guide up two tiers, with nothing to undo it.
  throw new Error((lastErr?.message || 'Request failed.') + (READS.has(action)
    ? " Google's script service is being flaky — hit Refresh and try again."
    : ' Google never confirmed it, but the change may still have gone through.' +
      ' Hit Refresh and check before doing it again.'));
}
