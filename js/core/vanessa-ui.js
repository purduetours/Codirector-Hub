...
export function resetVanessa() {
  resetWarmup(); resetVanessaData(); resetModel(); sendQueue = Promise.resolve(); open = false;
...
export function initVanessa() {
  ...
  const suggestions = [
    ...(inTraining() ? ['Who still needs an eval?', 'What are my evals?'] : []),
  ...
  panel.innerHTML = `<div class="v-head"><span><strong>Vanessa</strong><span class="sub">Answers from your loaded hub data</span></span>
    ...
    <div class="v-chips">${suggestions.map(s => `<button type="button" class="v-chip" data-question="${esc(s)}">${esc(s)}</button>`).join('')}</div>
